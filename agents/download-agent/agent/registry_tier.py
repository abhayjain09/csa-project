...
def edgar_lookup(company_ctx: dict, report_class: str, year: int | None) -> dict | None:
    ...
    if not cik:
        # A caller-supplied CIK is itself authoritative identity — a CIK maps
        # 1:1 to one SEC registrant. Trust it when it is a real SEC CIK, even
        # if the (possibly informal/abbreviated) company NAME did not
        # exact-match the SEC registrant name. This is what puts US annual/
        # proxy requests on the deterministic DEF 14A / 10-K path instead of
        # the looser web tiers that were grabbing the additional/soliciting
        # proxy. A wrong ticker is still rejected upstream (enrich_company_
        # identity) and never reaches here as a CIK.
        _payload_cik = _normalise_cik(company_ctx.get("cik"))
        if _payload_cik:
            _load_edgar_ticker_map()
            if _payload_cik in set(_EDGAR_TICKER_CACHE.values()):
                _log(f"[edgar] trusting caller-supplied SEC CIK {_payload_cik} "
                     f"for {company_ctx.get('name')!r}")
                cik = _payload_cik
    if not cik:
