...
locals {
  ...
  runtime_env = {
    ...
    BROWSER_NAV_MAX_PAGES                   = "25"
    BROWSER_RESOLVE_MAX_SECONDS             = "240"
    BROWSER_VISION_MODEL_ID                 = "us.amazon.nova-2-lite-v1:0"
    BROWSER_ALLOW_OFFDOMAIN_DOCS            = "true"
    ...
    # CLEAN_RERUN_DELETE_EXISTING is deliberately NOT listed here: the
    # AgentCore UpdateAgentRuntime API caps environmentVariables at 50 entries
    # and this map was already at that ceiling. The Python side already
    # defaults it to "false" when absent (os.environ.get(..., "false")), so
    # omitting it is behaviorally identical to setting it "false" — and the
    # feature stays reachable per-invocation via payload {"clean_rerun": true},
    # which needs no env var / no deploy at all. See _wipe_company_documents.
