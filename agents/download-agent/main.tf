...
data "aws_iam_policy_document" "agent" {
  statement {
    sid = "WriteReports"
    # s3:DeleteObject is required by _wipe_company_documents() for the opt-in
    # CLEAN_RERUN_DELETE_EXISTING path (agent/agent.py) — the agent only ever
    # deletes objects under a single resolved company's own prefix.
    actions   = ["s3:PutObject", "s3:GetObject", "s3:ListBucket", "s3:DeleteObject"]
    resources = [aws_s3_bucket.reports.arn, "${aws_s3_bucket.reports.arn}/*"]
  }
  statement {
    sid = "Provenance"
    # dynamodb:DeleteItem is required by the same clean-rerun wipe path.
...
resource "aws_bedrockagentcore_agent_runtime" "agent" {
  ...
  # ALL post-create updates are handled exclusively by null_resource.runtime_update
  # below via a direct CLI call to update-agent-runtime — never by this resource.
  # Previously ignore_changes only listed agent_runtime_artifact/environment_variables,
  # leaving role_arn / network_configuration / protocol_configuration / agent_runtime_name
  # unignored. This provider is new (see NOTE above) and its plan reports a diff on
  # one of those nested blocks on every apply even though the .tf config is
  # unchanged (a schema/normalization quirk, not a real config change) — so the
  # provider's OWN update-agent-runtime call fires too, consuming a SECOND
  # AgentCore runtime version on every deploy in addition to the one intentionally
  # created by null_resource.runtime_update (observed: 182 -> 184 per deploy.sh
  # run instead of 182 -> 183). `ignore_changes = all` makes this resource
  # create-only: no config drift here can ever trigger the native provider's
  # update path, so the null_resource's explicit CLI call is the ONLY thing that
  # can bump the version, exactly once per deploy. To deliberately change
  # role_arn/agent_runtime_name/etc. via Terraform in the future, temporarily
  # remove this lifecycle block or use `terraform apply -replace=...`.
  lifecycle {
    ignore_changes = all
  }
...
resource "null_resource" "runtime_update" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # Session keepalive ladder for slow (JS-heavy / WAF-challenged / large-IR)
      # sites. The agent's HEALTHY_BUSY ping loop already resets the idle timer,
      # but we ALSO pin the lifecycle explicitly so the ceiling is deterministic
      # and documented rather than relying on the 900s default:
      #   idleRuntimeSessionTimeout=1800 (30 min) — must sit ABOVE the
      #     orchestrator's AGENT_READ_TIMEOUT (1620s) so AgentCore never reaps a
      #     session the client is still legitimately waiting on.
      #   maxLifetime=3600 (1 h) — hard instance cap; ~2.4x a single query's
      #     QUERY_MAX_SECONDS=1500s deadline, so an in-flight query is never cut
      #     off mid-run. (Each report invoke uses its own ephemeral session.)
      # Must satisfy idleRuntimeSessionTimeout <= maxLifetime (AWS constraint).
      LIFECYCLE="idleRuntimeSessionTimeout=1800,maxLifetime=3600"
      ...
      # Try WITH --lifecycle-configuration first. If the installed AWS CLI predates
      # that flag it errors BEFORE any API call (or the API rejects the param) — a
      # failed update-agent-runtime creates NO new runtime version, so falling back
      # WITHOUT the flag wastes nothing and yields exactly one successful version.
      # We fall back (loudly) rather than hard-fail the deploy; keepalive then rests
      # solely on the HEALTHY_BUSY ping loop, which is already sufficient in practice.
      OUT=$(run_update --lifecycle-configuration "$LIFECYCLE")
      RC=$?
      if [ $RC -ne 0 ]; then
        echo "WARN: update-agent-runtime WITH --lifecycle-configuration failed (RC=$RC);" >&2
        echo "      AWS CLI may predate the flag. Retrying WITHOUT it — session keepalive" >&2
        echo "      will rely on the agent's HEALTHY_BUSY ping loop alone." >&2
        echo "$OUT" >&2
        OUT=$(run_update)
        RC=$?
      fi
