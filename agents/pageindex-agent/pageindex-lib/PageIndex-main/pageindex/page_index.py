...
async def toc_detector_single_page(content, model=None):
    ...
    response = await llm_completion(model=model, prompt=prompt)
...
async def extract_toc_content(content, model=None):
    ...
    response, finish_reason = await llm_completion(model=model, prompt=prompt, return_finish_reason=True)
    ...
    for attempt in range(max_attempts):
        new_response, finish_reason = await llm_completion(model=model, prompt=continue_prompt, chat_history=chat_history, return_finish_reason=True)
...
async def toc_transformer(toc_content, model=None):
    ...
    last_complete, finish_reason = await llm_completion(model=model, prompt=prompt, return_finish_reason=True)
    ...
    for attempt in range(max_attempts):
        ...
        new_complete, finish_reason = await llm_completion(model=model, prompt=continue_prompt, chat_history=chat_history, return_finish_reason=True)
...
async def add_page_number_to_toc(part, structure, model=None):
    ...
    current_json_raw = await llm_completion(model=model, prompt=prompt)
...
async def generate_toc_continue(toc_content, part, model=None, logger=None, _depth=0):
    logging.info(f"[phase2] generate_toc_continue depth={_depth} toc_items={len(toc_content)} part_chars={len(part)}")
    prompt = """
    ...
    response, finish_reason = await llm_completion(
        model=model, prompt=prompt,
        return_finish_reason=True,
        max_tokens=4096,
        _call_context=f"generate_toc_continue_depth{_depth}"
    )
    if finish_reason == 'finished':
        return extract_json(response)
    elif finish_reason == 'max_output_reached':
        # Output truncated — split the page group in half and retry each half.fv
        # Recurse until each half fits within the output limit.
        # Base case: if part is too small to split further, return what we have.
        page_blocks = part.split('<physical_index_')
        page_blocks = [p for p in page_blocks if p.strip()]
        if len(page_blocks) <= 2 or _depth >= 4:
            logging.warning(
                f"[phase2] max_output_reached — cannot split further "
                f"depth={_depth} pages={len(page_blocks)} returning partial"
            )
            return extract_json(response) if response else []
        mid      = len(page_blocks) // 2
        part_a   = '<physical_index_' + '<physical_index_'.join(page_blocks[:mid])
        part_b   = '<physical_index_' + '<physical_index_'.join(page_blocks[mid:])
        logging.info(
            f"[phase2] max_output_reached — splitting into halves "
            f"depth={_depth} pages_a={mid} pages_b={len(page_blocks)-mid}"
        )
        first_half  = await generate_toc_continue(toc_content,  part_a, model, logger, _depth+1)
        second_half = await generate_toc_continue(first_half,   part_b, model, logger, _depth+1)
        return second_half
    else:
        raise Exception(f'generate_toc_continue finish reason: {finish_reason}')
    
### add verify completeness
async def generate_toc_init(part, model=None):
    logging.info(f"[phase2] generate_toc_init part_chars={len(part)}")
    prompt = """
    ...
    response, finish_reason = await llm_completion(
        ...
        return_finish_reason=True,
        max_tokens=4096,
        _call_context="generate_toc_init"
    )
    if finish_reason == 'finished':
        return extract_json(response)
    else:
        raise Exception(f'generate_toc_init finish reason: {finish_reason}')

async def process_no_toc(page_list, start_index=1, model=None, logger=None):
    t0 = asyncio.get_event_loop().time()
    total_tokens = sum(p[1] for p in page_list)
    logger.info(
        f"[phase2] process_no_toc start pages={len(page_list)} "
        f"start_index={start_index} total_tokens={total_tokens}"
    )
    page_contents=[]
    token_lengths=[]
    ...
    # Use token-based group sizing (max 12000 input tokens per group) to prevent
    # generate_toc_continue() hitting max_output_reached on dense pages.
    # 12000 input tokens produces at most ~4000-6000 output tokens of section titles,
    # safely under the 8000 max_tokens set in generate_toc_continue().
    group_texts = page_list_to_group_text(page_contents, token_lengths, max_tokens=20000)
    logger.info(
        f"[phase2] process_no_toc groups={len(group_texts)} "
        f"avg_tokens_per_group={total_tokens//max(len(group_texts),1)}"
    )

    toc_with_page_number = await generate_toc_init(group_texts[0], model)
    logger.info(f"[phase2] generate_toc_init done items={len(toc_with_page_number)}")

    for gi, group_text in enumerate(group_texts[1:], 1):
        toc_with_page_number_additional = await generate_toc_continue(
            toc_with_page_number, group_text, model, logger)
        toc_with_page_number.extend(toc_with_page_number_additional)
        logger.info(
            f"[phase2] generate_toc_continue group={gi}/{len(group_texts)-1} "
            f"total_items={len(toc_with_page_number)}"
        )

    elapsed = round(asyncio.get_event_loop().time() - t0, 2)
    logger.info(
        f"[phase2] process_no_toc done items={len(toc_with_page_number)} elapsed={elapsed}s"
    )
...
async def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    ...
    if offset is None:
        logger.info(
            '[offset] calculate_page_offset returned None — '
            'no valid pairs found, defaulting offset to 0'
        )
        offset = 0
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    t0 = asyncio.get_event_loop().time()
    logger.info(
        f"[meta_processor] start mode={mode} pages={len(page_list)} "
        f"start_index={start_index} total_tokens={sum(p[1] for p in page_list)}"
    )
    ...
    logger.info({
        ...
        'toc_items': len(toc_with_page_number),
        'elapsed': elapsed,
    })
    if accuracy == 1.0 and len(incorrect_results) == 0:
        logger.info(f"[phase3] verify_toc passed accuracy=1.0 elapsed={elapsed}s")
        return toc_with_page_number
    if accuracy > 0.6 and len(incorrect_results) > 0:
        logger.info(
            f"[phase3] verify_toc partial accuracy={accuracy:.2f} "
            f"fixing {len(incorrect_results)} incorrect entries"
        )
        ...
        logger.info(
            f"[phase3] verify_toc failed accuracy={accuracy:.2f} "
            f"mode={mode} — falling back"
        )
        if mode == 'process_toc_with_page_numbers':
            ...
            raise Exception(
                f'Processing failed — all TOC paths exhausted. '
                f'pages={len(page_list)} start_index={start_index} '
                f'final_accuracy={accuracy:.2f}'
            )
        
 
async def process_large_node_recursively(node, page_list, opt=None, logger=None, _depth=0):
    # Depth guard — if meta_processor returns the same node structure (no
    # detectable section headers in content like tables/pension data), the
    # recursive call would loop indefinitely. Stop after MAX_DEPTH attempts.
    # The node itself stays in the tree as a leaf; no content is lost.
    MAX_DEPTH = 2
    if _depth >= MAX_DEPTH:
        return node

    # Defensive guard: end_index must be >= start_index before we slice page_list.
    # If a prior step produced end_index < start_index (same-page collision, bad
    # LLM output, or a corrupted end_physical_index passed to post_processing),
    # the slice would be empty, token_num would be 0, the size check would be False,
    # and the node would silently become a zero-page leaf — losing all its content
    # without any error. Clamp here so the slice always covers at least one page.
    if node.get('end_index') is None or node['end_index'] < node['start_index']:
        if logger:
            logger.info(
                f"[process_large_node] clamping bad end_index={node.get('end_index')} "
                f"to start_index={node['start_index']} for node '{node.get('title', '?')}'"
            )
        node['end_index'] = node['start_index']

    # Guard: clamp end_index so the page slice is never empty.
    # A bad end_index < start_index produces an empty slice, token_num=0,
    # the size check fails, and the node silently becomes a zero-page leaf.
    if node.get('end_index') is None or node['end_index'] < node['start_index']:
        if logger:
            logger.info(
                f"[process_large_node] clamping bad end_index={node.get('end_index')} "
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        logger.info(
            f"[phase5] splitting large node title='{node['title']}' "
            f"pages={node['end_index']-node['start_index']} "
            f"tokens={token_num} depth={_depth}"
        )
        ...
        # Ensure physical_index values are monotonically increasing so
        # post_processing() never produces end_index < start_index
        valid_node_toc_items = sanitize_physical_indices(valid_node_toc_items)

        if valid_node_toc_items and node['title'].strip() == valid_node_toc_items[0]['title'].strip():
            node['nodes'] = post_processing(valid_node_toc_items[1:], node['end_index'])
            new_end = valid_node_toc_items[1]['start_index'] if len(valid_node_toc_items) > 1 else node['end_index']
            # Guard: if post_processing produced a bad start_index (same-page
            # collision), new_end could be < node['start_index']. Clamp it.
            node['end_index'] = max(new_end, node['start_index'])
        else:
            node['nodes'] = post_processing(valid_node_toc_items, node['end_index'])
            new_end = valid_node_toc_items[0]['start_index'] if valid_node_toc_items else node['end_index']
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    pre_toc = getattr(opt, 'pre_extracted_toc', None)
    if pre_toc:
        # Pre-extracted TOC supplied by chunked indexing path.
        # Pass through meta_processor with mode='process_toc_with_page_numbers'
        # so offset calculation, verification and fallback paths all run correctly.
        # The TOC from extract_toc mode contains logical page numbers — without
        # offset calculation every section would be assigned the wrong physical page.
        logger.info(
            f"[phase1-2] pre_extracted_toc supplied items={len(pre_toc)} "
            f"— routing via process_toc_with_page_numbers for offset calculation"
        )

        # Convert pre_toc list into the toc_content string format that
        # meta_processor / process_toc_with_page_numbers expects.
        # Format: "1. Section Title ... page N" per entry.
        toc_content_lines = [
            f"{i+1}. {item['title']} ... {item.get('page', '')}"
            for i, item in enumerate(pre_toc)
        ]
        toc_content  = "\n".join(toc_content_lines)
        # toc_page_list: physical page indices of the TOC pages in the PDF.
        # Since we extracted the TOC externally, use page 0 as a placeholder —
        # process_toc_with_page_numbers uses this only to skip past the TOC
        # when sampling content pages for offset calculation.
        toc_page_list = [0]
    ...
    # Ensure physical_index values are monotonically increasing so
    # post_processing() never produces end_index < start_index
    valid_toc_items = sanitize_physical_indices(valid_toc_items)
...
def page_index_main(doc, opt=None):
    ...
    async def page_index_builder():
        ...
        # Phase 1-5: TOC detection, extraction, verification, post-processing, splitting
        logger.info(f"[phase1-5] tree_parser start pages={len(page_list)}")
        t1 = time.time()
        try:
            structure = await tree_parser(page_list, opt, doc=doc, logger=logger)
        except Exception as e:
            recoverable = (
                'Processing failed' in str(e)
                or 'all TOC paths exhausted' in str(e)
                or 'finish reason' in str(e)
                or isinstance(e, (AttributeError, KeyError, ValueError))
            )
            if recoverable:
                doc_name = get_pdf_name(doc)
                title    = doc_name.rsplit('.', 1)[0].replace('_', ' ').replace('-', ' ').title()
                logger.info(
                    f"[phase1-5] tree_parser failed — using single-node fallback "
                    f"pages={len(page_list)} error={type(e).__name__}: {e}"
                )
                structure = [{
                    "title":       title,
                    "node_id":     "0001",
                ...
                raise
        logger.info(f"[phase1-5] tree_parser done nodes={len(structure)} elapsed={round(time.time()-t1,2)}s")
        ...
        if opt.if_add_node_summary == 'yes':
            logger.info(f"[phase6] summary generation start")
            t6 = time.time()
            ...
            logger.info(f"[phase6] summary generation done elapsed={round(time.time()-t6,2)}s")
            if opt.if_add_doc_description == 'yes':
                ...
                logger.info(f"[total] page_index_builder done elapsed={round(time.time()-t_total,2)}s")
                return {
                    'doc_name': get_pdf_name(doc),
        ...
        # Final integrity check: log any nodes where end_index < start_index.
        # These are not fatal — the structure is still returned — but they
        # indicate a gap in the upstream guards and will cause zero-page slices
        # downstream. Logging them here makes the root cause traceable.
        def _audit_indices(nodes, path="root"):
            for node in nodes:
                si = node.get('start_index')
                ei = node.get('end_index')
                if si is not None and ei is not None and ei < si:
                    logger.info(
                        f"[audit] end_index < start_index: "
                        f"node_id={node.get('node_id')} title='{node.get('title')}' "
                        f"start={si} end={ei} path={path}"
                    )
                if node.get('nodes'):
                    _audit_indices(node['nodes'], path=f"{path}/{node.get('node_id','?')}")
        _audit_indices(structure)
