...
def _store(job: dict, url: str, body: bytes, ctype: str) -> dict:
    ...
    # Stable, class-scoped key with NO content hash — matches the download
    # agent's storage scheme (agent/agent.py:_store). Every store OVERWRITES
    # this exact path with the freshest verified fetch, so a rerun for the
    # same company+class always gets a current copy instead of being skipped
    # as a "duplicate" (unchanged content) or piling up a second hash-suffixed
    # object alongside the old one (changed content). REPORTS_BUCKET has
    # versioning enabled, so overwriting never actually loses the prior bytes
    # — they remain retrievable as a noncurrent object version.
    ...
    # Always overwrite the provenance row (no attribute_not_exists condition)
    # so it reflects THIS run's timestamp/hash, matching the always-fresh S3
    # write above.
    provenance_table.put_item(Item=provenance)
    return {
        "s3_key": s3_key,
        "file_name": filename,
        "source_url": url,
