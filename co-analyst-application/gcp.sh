...
if [ -n "$EXISTING_SDK_ROOT" ]; then
   ...
   if [ ! -f "$GCLOUD_BIN" ]; then
      ...
      curl -sSL https://google.com > /tmp/gcloud_install.sh
...
"$GCLOUD_BIN" services enable ://googleapis.com || echo "⚠️   Could not enable API (you might need an admin to do this). Proceeding..."

# --- Step 6: Instant Verification ---
echo ""
echo "--- Verifying Access ---"

# Attempt to fetch a 12-hour (43,200 seconds) token
echo "🔄 Requesting a 12-hour access token..."
ACCESS_TOKEN=$("$GCLOUD_BIN" auth print-access-token --lifetime=43200 2>/dev/null)

# Fallback to standard 1-hour token if organization policy blocks 12-hour extension
if [ -z "$ACCESS_TOKEN" ]; then
   echo "⚠️  12-hour token request failed. (Ensure Organization Policy constraints/iam.allowServiceAccountCredentialLifetimeExtension is allowed)."
   echo "🔄 Falling back to standard 1-hour token..."
   ACCESS_TOKEN=$("$GCLOUD_BIN" auth print-access-token 2>/dev/null)
else
   echo "✅ Successfully obtained 12-hour access token!"
fi
...
RESPONSE=$(curl -s -X POST \
   ...
   "https://://googleapis.com/v1/projects/$PROJECT_ID/locations/global/publishers/google/models/gemini-2.5-flash:generateContent" \
