...
terraform apply -input=false -lock=false -auto-approve -target=aws_ecr_repository.app
...
step "4/4  Full apply"
cd "$TF_DIR"
terraform apply -input=false -lock=false -auto-approve -var="image_tag=$TAG"
